...
export default function Aside () {
  ...
  // const position= isPath ? 'md:fixed' : 'md:relative'
  return (
    <nav className={`border-r border-neutral-content ${open?'fixed':'relative'} md:relative z-40 `}>
      <div className={` sticky top-nav mt-nav`}>
        <div className={`${!open ? ' max-w-12 -left-[0px]' : 'w-auto max-w-96 left-0'} relative duration-0 overflow-hidden`}>
          <div className="flex">
            <div className={`${!open ? 'p-1 py-3' : 'p-3'} h-dvh-nav overflow-y-auto`}>
              <div onClick={e=> setOpen(!open)} className="w-full flex justify-end items-center">
...
const Swap= ()=> {
  ...
  return (
    <div className="">
      <input type="checkbox" onChange={e=> null} checked={checked} className="theme-controller hidden" value="dim" />
      
      <label className="swap swap-rotate">
        <input onChange={e=>setChecked(!checked)} type="checkbox" checked={!checked} className="theme-controller" value="nord" />
